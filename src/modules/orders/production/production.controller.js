...
// Ready = stock + produced + procured (same formula as the frontend)
const calculateReadyQty = ({ availableStockQty, producedQty, procuredQty }) => {
  return toNumber(availableStockQty) + toNumber(producedQty) + toNumber(procuredQty);
};

const calculatePendingQty = ({ orderQty, readyQty }) => Math.max(toNumber(orderQty) - toNumber(readyQty), 0);

const buildProductionStatus = ({ orderQty, availableStockQty, producedQty, procuredQty, explicitStatus }) => {
  const safeExplicit = normalizeEnum(explicitStatus, allowedProductionStatuses, "");
  const readyQty = calculateReadyQty({ availableStockQty, producedQty, procuredQty });

  if (safeExplicit === "hold") return "hold";
  if (orderQty > 0 && readyQty >= orderQty) return "ready";
  if (readyQty > 0) return "partially_ready";
  if (toNumber(producedQty) > 0 || toNumber(procuredQty) > 0) return "in_progress";
  return "not_started";
};
...
const saveProductionForItem = async ({ item, body, user }) => {
  ...
  const productionStatus = buildProductionStatus({ orderQty, availableStockQty, producedQty, procuredQty, explicitStatus: body.production_status });
  const priority = normalizeEnum(body.priority || item.order_priority, allowedPriorities, "normal");
  const remarks = body.remarks || null;
